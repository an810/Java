...
public class Input
{
    public static void input()
    {
        ...
        Configs.distance = new double[dim][dim];
        ...
        for (int i = 0; i < dim; i++)
        {
            for (int j = 0; j < dim; j++)
            {
                if (i == j)
                {
                    Configs.distance[i][j] = 0.0;
                }
                else
                {
                    Configs.distance[i][j] = Math.sqrt(Math.pow(Configs.xCor[j] - Configs.xCor[i], 2) + Math.pow(Configs.yCor[j] - Configs.yCor[i], 2));
                }
            }
        }
    }
